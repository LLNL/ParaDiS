/**************************************************************************
 *
 *      Module:       MobilityLaw_BCC_Va_nl_planar.c
 *      Description:  
 *
 *
 ...
 *            FscaleScrew()
 *                
 *      Last Modified: 11/18/2008 gh - Original version based on the
 *                                     Arsenlis matlab code mobbccVanlplanar_dbg2.m
 *
 ***************************************************************************/
#include <stdio.h>
#include <math.h>
...
static void FindScrewIndex(real8 force[3], real8 screwb[MAX_NSCREW][3],
                           int numScrews, int ind[MAX_NSCREW])
{
        int   i, j;
        real8 screwNormal, planeNormal, val;
        real8 linedir[3], fplanetest[3], planedirmat[3][3];

        for (i = 0; i < numScrews; i++) {
            screwNormal = sqrt(screwb[i][0]*screwb[i][0] +
                               screwb[i][1]*screwb[i][1] +
                               screwb[i][2]*screwb[i][2]);

            linedir[0] = screwb[i][0] / screwNormal;
            linedir[1] = screwb[i][1] / screwNormal;
            linedir[2] = screwb[i][2] / screwNormal;

            planedirmat[0][0] =  0.0;
            planedirmat[0][1] =  linedir[2];
            planedirmat[0][2] = -linedir[1];

            planedirmat[1][0] = -linedir[2];
            planedirmat[1][1] =  0.0;
            planedirmat[1][2] =  linedir[0];

            planedirmat[2][0] =  linedir[1];
            planedirmat[2][1] = -linedir[0];
            planedirmat[2][2] =  0.0;

            for (j = 0; j < 3; j++) {
                planeNormal = sqrt(planedirmat[j][0]*planedirmat[j][0] +
                                   planedirmat[j][1]*planedirmat[j][1] +
                                   planedirmat[j][2]*planedirmat[j][2]);
                planedirmat[j][0] = planedirmat[j][0] / planeNormal;
                planedirmat[j][1] = planedirmat[j][1] / planeNormal;
                planedirmat[j][2] = planedirmat[j][2] / planeNormal;
            }

            Matrix33Vector3Multiply(planedirmat, force, fplanetest);

            val = fabs(fplanetest[0]);
            ind[i] = 0;

            for (j = 1; j < 3; j++) {
                if (fabs(fplanetest[j])>val) {
                    val = fabs(fplanetest[j]);
                    ind[i] = j;
                }
            }
        }
...
static void ScrewDrag(Param_t *param, real8 finit[3], real8 vel[3],
                      real8 burg[3], int ind, real8 dfsdragdv[3][3])
{
        int   i, m, n;
        real8 tmp, tmp2[2];
        real8 burgnormal, Bsline;
        real8 sqrt2, sqrt_half;
        real8 fout1, fout2;
        real8 dfdv1, dfdv2;
        real8 d2fdv21, d2fdv22;
        real8 vplane[2], fplane[2];
        real8 vproj[3], fproj[3];
        real8 linedir[3], linedirTM[3][3];
        real8 tmp22[2][2], invTmp22[2][2], tmp23[2][3], tmp33[3][3];
        real8 vects[2][3];
...
/*
 *      This normalizes the 112 glide directions for the screw
 *      dislocation in the 110 planes.
 */
        sqrt_half = sqrt(0.5);

        for (m = 0; m < 3; m++) {
            for (n = 0; n < 3; n++) {
                glidedirmat[m][n] = glidedirmat[m][n] * sqrt_half;
            }
        }

        if (ind == 0) {

            tmp22[0][0] = glidedirmat[1][1];
            tmp22[0][1] = glidedirmat[1][2];
            tmp22[1][0] = glidedirmat[2][1];
            tmp22[1][1] = glidedirmat[2][2];

            Matrix22_Inverse(invTmp22, tmp22);

            Matrix22_Vmul(vplane, invTmp22, &vproj[1] );   // vplane = invTmp22 * &vproj[1]
            Matrix22_Vmul(fplane, invTmp22, &fproj[1] );   // fplane = invTmp22 * &fproj[1]

            FscaleScrew(param, fabs(fplane[0]), fabs(vplane[0]), burg, &fout1, &dfdv1, &d2fdv21);
            FscaleScrew(param, fabs(fplane[1]), fabs(vplane[1]), burg, &fout2, &dfdv2, &d2fdv22);

            Bsline = 0.5 * (dfdv1+dfdv2) * 1.0e-2;
            ...
            Matrix33Vector3Multiply(dfsdragdv, vel, finit);

            for (i = 0; i < 3; i++) {
                finit[i] += fout1 * glidedirmat[1][i] * Sign(vplane[0]) +
                            fout2 * glidedirmat[2][i] * Sign(vplane[1]);
            }

            tmp23[0][0] = glidedirmat[1][0];
            tmp23[0][1] = glidedirmat[1][1];
            tmp23[0][2] = glidedirmat[1][2];
            tmp23[1][0] = glidedirmat[2][0];
            tmp23[1][1] = glidedirmat[2][1];
            tmp23[1][2] = glidedirmat[2][2];

            MatrixMult((real8 *)invTmp22, 2, 2, 2,
                       (real8 *)tmp23, 3, 3,
                       (real8 *)vects, 3);

            Matrix33_VVt_Mul(tmp33,glidedirmat[1], vects[0]);

            for (m = 0; m < 3; m++) {
                for (n = 0; n < 3; n++) {
                    dfsdragdv[m][n] += sqrt2 * dfdv1 * tmp33[m][n];
                }
            }

            Matrix33_VVt_Mul(tmp33,glidedirmat[2], vects[1]);
            for (m = 0; m < 3; m++) {
                for (n = 0; n < 3; n++) {
                    dfsdragdv[m][n] += sqrt2 * dfdv2 * tmp33[m][n];
                }
            }

        } else if (ind == 1) {

            tmp22[0][0] = glidedirmat[0][0];
            tmp22[0][1] = glidedirmat[0][2];
            tmp22[1][0] = glidedirmat[2][0];
            tmp22[1][1] = glidedirmat[2][2];

            Matrix22_Inverse(invTmp22, tmp22);        // invTmp22 = Inverse(tmp22)

            tmp2[0] = vproj[0];
            tmp2[1] = vproj[2];
            Matrix22_Vmul(vplane, invTmp22, tmp2 );   // vplane = invTmp22 * tmp2

            tmp2[0] = fproj[0];
            tmp2[1] = fproj[2];
            Matrix22_Vmul(fplane, invTmp22, tmp2 );   // fplane = invTmp22 * tmp2
            ...
            for (i = 0; i < 3; i++) {
                finit[i] += fout1 * glidedirmat[0][i] * Sign(vplane[0]) +
                            fout2 * glidedirmat[2][i] * Sign(vplane[1]);
            }

            tmp23[0][0] = glidedirmat[0][0];
            tmp23[0][1] = glidedirmat[0][1];
            tmp23[0][2] = glidedirmat[0][2];
            tmp23[1][0] = glidedirmat[2][0];
            tmp23[1][1] = glidedirmat[2][1];
            tmp23[1][2] = glidedirmat[2][2];
            ...
            Matrix33_VVt_Mul(tmp33,glidedirmat[0], vects[0]);
            for (m = 0; m < 3; m++) {
                for (n = 0; n < 3; n++) {
                    dfsdragdv[m][n] += sqrt2 * dfdv1 * tmp33[m][n];
                }
            ...
            tmp22[0][0] = glidedirmat[0][0];
            tmp22[0][1] = glidedirmat[0][1];
            tmp22[1][0] = glidedirmat[1][0];
            tmp22[1][1] = glidedirmat[1][1];

            Matrix22_Inverse(invTmp22, tmp22);         // invTmp22 = Inverse(tmp22)

            Matrix22_Vmul(vplane, invTmp22, vproj );   // vplane = invTmp22 * vproj
            Matrix22_Vmul(fplane, invTmp22, fproj );   // fplane = invTmp22 * fproj
            ...
            for (i = 0; i < 3; i++) {
                finit[i] += fout1 * glidedirmat[0][i] * Sign(vplane[0]) +
                            fout2 * glidedirmat[1][i] * Sign(vplane[1]);
            }

            tmp23[0][0] = glidedirmat[0][0];
            tmp23[0][1] = glidedirmat[0][1];
            tmp23[0][2] = glidedirmat[0][2];
            tmp23[1][0] = glidedirmat[1][0];
            tmp23[1][1] = glidedirmat[1][1];
            tmp23[1][2] = glidedirmat[1][2];
            ...
            Matrix33_VVt_Mul(tmp33,glidedirmat[1],vects[1]);
            for (m = 0; m < 3; m++) {
                for (n = 0; n < 3; n++) {
                    dfsdragdv[m][n] += sqrt2 * dfdv2 * tmp33[m][n];
                }
        ...
        tmp = DotProduct(linedir, vel);

        Matrix33_VVt_Mul(tmp33,linedir, vects[0]);
        for (m = 0; m < 3; m++) {
            for (n = 0; n < 3; n++) {
                dfsdragdv[m][n] += tmp33[m][n] * (0.5 * sqrt2 * d2fdv21 *
                                                  1.0e-02 * tmp *
                                                  Sign(vplane[0]));
            }
        }

        Matrix33_VVt_Mul(tmp33,linedir, vects[1]);
        for (m = 0; m < 3; m++) {
            for (n = 0; n < 3; n++) {
                dfsdragdv[m][n] += tmp33[m][n] * (0.5 * sqrt2 * d2fdv22 *
                                                  1.0e-02 * tmp *
                                                  Sign(vplane[1]));
            }
        }
...
/**************************************************************************
 *
 *      Function:     MobilityLaw_BCC_Va_nl_node
...
/*
 *      In this mobility module, the glide constraints are incorporated
 *      into the drag matrix through the edge and screw drag functions.
 */
 ...
 */
{
        ...
        home->param->deltaTT = 1.310467200000000e-15;

        numNbrs = 3;

        fn[0] =  4.343904972133274e+09;
        fn[1] = -3.405705054703005e+10;
        fn[2] =  1.409910758434409e+10;

        vtest[0] = -2.982234532871480e+10;
        vtest[1] = -3.500862836695335e+10;
        vtest[2] = -3.552961356822504e+10;

        x[0] =  4.489138037858795e+03;
        y[0] =  3.426379309172341e+03;
        z[0] =  3.269484964559607e+03;

        x[1] = 4.492845711452617e+03;
        y[1] = 3.364461976659808e+03;
        z[1] = 3.267965151347494e+03;

        x[2] = 4.378721220411031e+03;
        y[2] = 3.312725665660264e+03;
        z[2] = 3.147577541243562e+03;

        x[3] = 4.512608919802626e+03;
        y[3] = 3.453208337753596e+03;
        z[3] = 3.301282008911734e+03;
 ...
 */
        ...
        FindScrewIndex(fn, screwb, numscrews, screwIndex);
 ...
 *              Debug code only.
 */
                if ((home->param->deltaTT < 5.0e-14) && dumpDebugData) {
