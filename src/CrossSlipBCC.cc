 ...
 */
                ...
                if (seg1_is_screw || seg2_is_screw || bothseg_are_screw) {
                    ...
                    real8  tmp33[3][3];
                    real8  glideDirCrystal[3][3], glideDirLab[3][3];
 ...
 */
                    Matrix33_VVt_Mul(tmp33,burgCrystal, burgCrystal);

                    for (m = 0; m < 3; m++) {
                        for (n = 0; n < 3; n++) {
                            glideDirCrystal[m][n] = ((m==n)-tmp33[m][n]) *
                                                    sqrt(1.5);
                        }
                    }

                    // Print3x3("glideDir",glideDirCrystal);exit(0);
                            ...
                            if (!pinned1) {
                               ...
                               (fabs(tmp3C[fplane]) >
                                (weightFactor*fabs(tmp3C[plane2]) +
                                 fnodeThreshold))){
/*
 *                      Zipper condition met for second segment.  If the second
 *                      neighbor is either not pinned or pinned but already
 *                      sufficiently aligned, this is a potential cross-slip
 *                      event.
 */
