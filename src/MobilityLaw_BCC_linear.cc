 ...
 *      ==============================================================
 *
 *      The BCC linear mobility law computes the velocity of a node
 *      solution of
 *
 ...
 *      BCC is linear, this means that this system can be re-written
 *      as B . v = F and by inverting the 3x3 matrix B, v could be
 *      found directly.
 *
 *      It is assumed that the user has drag coefficients for the
 *      the BCC material. The drag coefficients for the different
 *      BCC planes should be provided for edge and screw components.
 *
...
/**************************************************************************
 *
 *      Function:     MobilityLaw_BCC_linear
 ...
 */
        ...
        dragLine  = 1.0e-4 * MIN(dragScrew, dragEdge);
 ...
 */
        if (numNbrs > MAX_NBRS) {
            Fatal("Mobility_BCC_linear: Node segment count (%d) exceeds\n"
 ...
 */
	  ...
	  //printf("maxIterCount = %d forceError=%f /%f\n",count,forceError,fErrorTol);
            return(1);
        }
