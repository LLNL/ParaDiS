/**************************************************************************
 *
 *      Module:       Mobility_BCC_Ta_nl_b_planar.c
 ...
 *                    This is a variant of BCC_Ta_nl in which the
 *                    EdgeDrag() and ScrewDrag() routines have been
 *                    modified to remove bsline and beline in order
 *                    to replace the undesireable functions of beline and 
 *                    bsline with the non-linear edge and screw scalar
 *                    functions, as well as the addition of a multiplier.
 *                    Also, movement is restricted to the segment glide
 *                    planes.
 *
 ...
 *                    Based on the Arsenlis mobbccTanl5altc3.m matlab code.
 *      
 *
...
static void FscaleScrew(Param_t *param, real8 fsinit, real8 vin, real8 burg[3],
                        real8 *fout, real8 *dfdv, real8 *dfdvlin,
                        real8 *d2fdv2);
                     ...
                     real8 dfedragdv[3][3])
{
        ...
        real8 vmag2, fout2, df2dv, df2dvlin, d2f2dv2, factor;
        real8 Beclimb;
        real8 ftest;
 ...
 */
        ...
        FscaleScrew(param, 0, vmag2, burg, &fout2, &df2dv, &df2dvlin, &d2f2dv2);
...
static void ScrewDrag(Param_t *param, real8 finit[3], real8 vel[3],
                      real8 burg[3], real8 screwNorm[3], real8 dfsdragdv[3][3])
{
        int    i, m, n;
        real8  factor, burgNormal;
        real8  vMag, vMag2, fTest, Bsclimb;
        real8  fOut, dfdv, d2fdv2, dfdvlin;
        real8  fOut2, df2dv, d2f2dv2, df2dvlin;
        real8  lineDir[3], climbDir[3], glideDir[3], tmp3[3];
        real8  lineDirTM[3][3], climbDirTM[3][3], glideDirTM[3][3];
/*
 *      May need to play with factor to see how it behaves.   factor should
 *      be greater than 0, but no larger than 1
 */
        factor = 1.0e-04;
        ...
        climbDir[0] = screwNorm[0];
        climbDir[1] = screwNorm[1];
        climbDir[2] = screwNorm[2];

        cross(lineDir, climbDir, glideDir);
        ...
        FscaleEdge(param, 0, vMag2, burg, &fOut2, &df2dv, &df2dvlin, &d2f2dv2);

        Bsclimb = dfdvlin;

        for (m = 0; m < 3; m++) {
            for (n = 0; n < 3; n++) {
                dfsdragdv[m][n] = Bsclimb * climbDirTM[m][n];
            }
        }
        ...
        for (i = 0; i < 3; i++) {
            finit[i] = tmp3[i] + (fOut * glideDir[i]) +
        ...
        for (m = 0; m < 3; m++) {
            for (n = 0; n < 3; n++) {
                dfsdragdv[m][n] += dfdv  * glideDirTM[m][n] +
                                   df2dv * lineDirTM[m][n] * factor;
            }
        }
...
/**************************************************************************
 *
 *      Function:     Mobility_BCC_Ta_nl_b_planar
...
int Mobility_BCC_Ta_nl_b_planar(Home_t *home, Node_t *node, MobArgs_t *mobArgs)
{
        ...
        int     numjunct, thisjunct, numNorms;
        ...
        real8   junctnorm[MAX_NBRS  ][3]={{0.0}}, screwnorm [MAX_NBRS  ][3]={{0.0}};
        real8   tmp;
        real8   tmp33[3][3];
        real8   correction[3];
        ...
        real8   EpsC0;
        real8   tmpNormal, dpnmjn, dpnmsn, dpnmen;
        Param_t *param;
        Node_t  *nbrNode;


        real8 (*invdferrordv)[3]    =  mobArgs->invDragMatrix;
        real8 (*normMatrix)[3]      =  mobArgs->glideConstraints;
 ...
 */
            if (param->useLabFrame) {
                ...
                Matrix33Vector3Multiply(param->rotMatrixInverse,ndir[i],ndirRot);
        ...
        for (i = 0, iAll = 0; iAll < numNbrs; i++, iAll++) {
            ...
            if (fabs(burgProd) <= eps) {
                ...
                for (j = 0; j < numjunct; j++) {
                    mag1 = DotProduct(burg[i], junctb[j]) /
                           (Normal(burg[i]) * Normal(junctb[j]));
                    mag2 = DotProduct(linedir[i], junctdir[j]) /
                           (Normal(linedir[i]) * Normal(junctdir[j]));
                    mag3 = DotProduct(ndir[i], junctnorm[j]) /
                           (Normal(ndir[i]) * Normal(junctnorm[j]));
                    if ((fabs(fabs(mag1)-1.0) < eps) &&
                        (fabs(fabs(mag2)-1.0) < eps) &&
                        (fabs(fabs(mag3)-1.0) < eps)) {
                        thisjunct = j;
                    }
                }
                ...
                if (thisjunct < 0) {
                    ...
                    junctnorm[numjunct][0] = ndir[i][0];
                    junctnorm[numjunct][1] = ndir[i][1];
                    junctnorm[numjunct][2] = ndir[i][2];
            ...
            if (fabs(burgProd) > eps) {
                ...
                if (screwExists[i]) {
                    ...
                    for (j = 0; j < numscrews; j++) {
                        mag = DotProduct(burg[i], screwb[j]) /
                              (Normal(burg[i]) * Normal(screwb[j]));
                        mag2 = DotProduct(ndir[i], screwnorm[j]) /
                               (Normal(ndir[i]) * Normal(screwnorm[j]));
                    ...
                    if (thisScrew < 0) {
                        ...
                        screwnorm[numscrews][0] = ndir[i][0];
                        screwnorm[numscrews][1] = ndir[i][1];
                        screwnorm[numscrews][2] = ndir[i][2];
                ...
                if (edgeExists[i]) {
                    ...
                    for (j = 0; j < numedges; j++) {
                        mag1 = DotProduct(ndir[i], edgenorm[j]) /
                               (Normal(ndir[i]) * Normal(edgenorm[j]));
                        mag2 = DotProduct(burg[i], edgeb[j]) /
...
/*
 *      Set up normMatrix and count the number of normals
 */

        numNorms = 0;

        i = 0;
        while ((numNorms < 3) && (i < numjunct)) {
            tmpNormal = Normal(junctnorm[i]);
            switch (numNorms) {
                case 0:
                    normMatrix[0][0] = junctnorm[i][0] / tmpNormal;
                    normMatrix[0][1] = junctnorm[i][1] / tmpNormal;
                    normMatrix[0][2] = junctnorm[i][2] / tmpNormal;
                    numNorms = 1;
                    break;

                case 1:
                    mag1 = DotProduct(normMatrix[0], junctnorm[i]) /
                           (Normal(normMatrix[0]) * Normal(junctnorm[i]));
                    if (fabs(fabs(mag1) - 1.0) > eps) {
                        dpnmjn = DotProduct(normMatrix[0], junctnorm[i]);
                        normMatrix[1][0] = junctnorm[i][0] - dpnmjn*normMatrix[0][0];
                        normMatrix[1][1] = junctnorm[i][1] - dpnmjn*normMatrix[0][1];
                        normMatrix[1][2] = junctnorm[i][2] - dpnmjn*normMatrix[0][2];
                        Normalize(&normMatrix[1][0], &normMatrix[1][1], &normMatrix[1][2]);
                        cross(normMatrix[0], normMatrix[1], normMatrix[2]);
                        numNorms = 2;
                    }
                    break;

                case 2:
                    mag1 = DotProduct(normMatrix[2], junctnorm[i]) /
                           (Normal(normMatrix[2]) * Normal(junctnorm[i]));
                    if (fabs(mag1) > eps) {
                        numNorms = 3;
                    }
                    break;
        ...
        i = 0;
        while ((numNorms < 3) && (i < numscrews)) {
            tmpNormal = Normal(screwnorm[i]);
            switch (numNorms) {
                case 0:
                    normMatrix[0][0] = screwnorm[i][0] / tmpNormal;
                    normMatrix[0][1] = screwnorm[i][1] / tmpNormal;
                    normMatrix[0][2] = screwnorm[i][2] / tmpNormal;
                    numNorms = 1;
                    break;

                case 1:
                    mag1 = DotProduct(normMatrix[0], screwnorm[i]) /
                           (Normal(normMatrix[0]) * Normal(screwnorm[i]));
                    if (fabs(fabs(mag1) - 1.0) > eps) {
                        dpnmsn = DotProduct(normMatrix[0], screwnorm[i]);
                        normMatrix[1][0] = screwnorm[i][0] - dpnmsn*normMatrix[0][0];
                        normMatrix[1][1] = screwnorm[i][1] - dpnmsn*normMatrix[0][1];
                        normMatrix[1][2] = screwnorm[i][2] - dpnmsn*normMatrix[0][2];
                        Normalize(&normMatrix[1][0], &normMatrix[1][1], &normMatrix[1][2]);
                        cross(normMatrix[0], normMatrix[1], normMatrix[2]);
                        numNorms = 2;
                    }
                ...
                case 2:
                    mag1 = DotProduct(normMatrix[2], screwnorm[i]) /
                           (Normal(normMatrix[2]) * Normal(screwnorm[i]));
                    if (fabs(mag1) > eps) {
                        numNorms = 3;
                    }
                    break;
        ...
        i = 0;
        while ((numNorms < 3) && (i < numedges)) {
            tmpNormal = Normal(edgenorm[i]);
            switch (numNorms) {
                case 0:
                    normMatrix[0][0] = edgenorm[i][0] / tmpNormal;
                    normMatrix[0][1] = edgenorm[i][1] / tmpNormal;
                    normMatrix[0][2] = edgenorm[i][2] / tmpNormal;
                    numNorms = 1;
                    break;

                case 1:
                    mag1 = DotProduct(normMatrix[0], edgenorm[i]) /
                           (Normal(normMatrix[0]) * Normal(edgenorm[i]));
                    if (fabs(fabs(mag1) - 1.0) > eps) {
                        dpnmen = DotProduct(normMatrix[0], edgenorm[i]);
                        normMatrix[1][0] = edgenorm[i][0] - dpnmen*normMatrix[0][0];
                        normMatrix[1][1] = edgenorm[i][1] - dpnmen*normMatrix[0][1];
                        normMatrix[1][2] = edgenorm[i][2] - dpnmen*normMatrix[0][2];
                        Normalize(&normMatrix[1][0], &normMatrix[1][1], &normMatrix[1][2]);
                        cross(normMatrix[0], normMatrix[1], normMatrix[2]);
                        numNorms = 2;
                    }
                ...
                case 2:
                    mag1 = DotProduct(normMatrix[2], edgenorm[i]) /
                           (Normal(normMatrix[2]) * Normal(edgenorm[i]));
                    if (fabs(mag1) > eps) {
                        numNorms = 3;
                    }
                    break;
 ...
 */
            ...
            for (i = 0; i < numscrews; i++) {
                ScrewDrag(param, fscrew[i], vtest, screwb[i], screwnorm[i], dfsdv);
                for (m = 0; m < 3; m++) {
...
//  *      to the new values after removing velocity in the glide plane
//  *      normals.
//  *
//  *      If needed, rotate the velocity vector back to the laboratory frame
//  *      from the crystal frame.
//  */
//         ApplyConstraintsToVelocity(*numGlideConstraints, normMatrix, vtest);
