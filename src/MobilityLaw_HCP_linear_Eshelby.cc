/**************************************************************************
 *
 *      Module:       Mobility_HCP_linear_Eshelby.c
 *      Author:       Sylvie Aubry, Chaoming Yang
...
int Mobility_HCP_linear_Eshelby(Home_t *home, Node_t *node, MobArgs_t *mobArgs)
{
        ...
        real8 alpha;
        real8 dragJunctionLineE, dragJunctionGlideE, dragLineE, dragClimbE;
        real8 dragGlideScrewE[MAX_NBRS]; 
        real8 dragGlideEdge1E[MAX_NBRS];
        real8 dragGlideEdge2E[MAX_NBRS];
#endif
...
#ifdef ESHELBY
        dragJunctionLineE  = alpha * dragJunctionLine;
        dragJunctionGlideE = alpha * dragJunctionGlide;
#endif
 ...
 */
            if (param->useLabFrame) {
                ...
                Matrix33Vector3Multiply(param->rotMatrixInverse, rt,
                                        rtRot);
                Matrix33Vector3Multiply(param->rotMatrixInverse, plane,
                                        planeRot);
 ...
 */
                    if (numLimits == 0){
                        ...
                        if (newRegion == numLimits){
                            ...
                            Fatal("numLimits (%d) exceeds max of %d %s:%d",
                                   numLimits, NUMLIMITS_MAX, __FILE__, __LINE__);
                            }
                        }
                    }
...
#ifdef ESHELBY
		dragGlideScrewE[i] = alpha * dragGlideScrew[i];
#endif			      
/* 
 ...
 */
		...
		typePlane2 = planeType[start+edgeIndex2];
                ...
                if (bIndex < 3) {
                    ...
                    //
                    if (typePlane1==1) {
                        dragGlideEdge1[i] = CdragEdge[0];
                    } else {
                        dragGlideEdge1[i] = CdragEdge[1];
                    }

                    if (typePlane2==1) {
                        dragGlideEdge2[i] = CdragEdge[0];
                    } else {
                    ...
                    dragGlideEdge1E[i] = alpha * dragGlideEdge1[i];
                    dragGlideEdge2E[i] = alpha * dragGlideEdge2[i];
#endif
 ...
 */
                if (junctionExists[i]) {
                    ...
                    /*** inside particles ***/ 
                    JunctionDrag(vTest, lineDir[i], dragJunctionLineE, 
                                dragJunctionGlideE, fJunction, dfJunctiondv);
 ...
 */
                    if (screwExists[i]) {
                        ...
                        /*** Inside particles ***/ 
                        ScrewDrag2(vTest, burg[i], ndir[i],
                                   dragClimbE, dragLineE, dragGlideScrewE[i],
                                   fScrew, dfScrewdv);
 ...
 */
                        ...
                        /***  Inside particles ***/
                        EdgeDrag(vTest, burg[i], edgeDir1[i],
                                 dragClimbE, dragLineE, dragGlideEdge1E[i],
                                 fEdge, dfEdgedv);
 ...
 */
                        ...
                        /*** Inside particles ***/ 
                        EdgeDrag(vTest, burg[i], edgeDir2[i],
                                 dragClimbE, dragLineE, dragGlideEdge2E[i],
                                 fEdge, dfEdgedv);
 ...
 */
        ...
        if ((isnan(node->vX) || isinf(node->vX)) ||
            ...
            PrintNode(node);
            Fatal("Mobility_HCP_linear: (%d, %d) velocity error.",
