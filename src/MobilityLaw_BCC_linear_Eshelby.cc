/**************************************************************************
 *
 *      Module:       Mobility_BCC_linear_Eshelby.c
 *      Author:       Sylvie Aubry
 *      Description:  Contains functions for calculating generic isotropic
 *                    linear mobility for BCC materials in a non-linear
 *                    fashion. Routine similar to HCP_linear.

 *      ==============================================================
 *      Includes public functions:
 *                    MobilityLaw_BCC_linear_Eshelby()
 *
 *      See MobilityLaw_BCC_linear() for details.
 *
...
/**************************************************************************
 *
 *      Function:     MobilityLaw_BCC_linear_Eshelby
...
#ifdef ESHELBY
        // Eshelby particles specific variables.
        real8 pos1[3], pos2[3];
        real8 partPos[3];
        real8 factor[MAX_NBRS];
        real8   ratioList[2][NUMLIMITS_MAX];
        SegPartIntersect_t *intersection;
        EInclusion_t *inclusion;
        real8 alpha, dragScrewE, dragEdgeE;
        real8 dragLineE, dragClimbE;
#endif
/*
 ...
 */
        ...
        dragLine  = 1.0e-2 * MIN(dragScrew, dragEdge);

#ifdef ESHELBY
/*     
 *      Drag parameters inside the particles chosen to be a factor
 *      alpha of the drag parameters outside the particles. 
 */
        alpha = param->MobEshelbyResist;

        dragScrewE  = alpha*dragScrew;
        dragEdgeE   = alpha*dragEdge;
        dragClimbE  = alpha*dragClimb;
        dragLineE   = alpha*dragLine;
#endif
/*
 ...
 */
        if (numNbrs > MAX_NBRS) {
            Fatal("Mobility_BCC_linear_Eshelby: Node segment count (%d) exceeds\n"
...
#ifdef ESHELBY
/*
 *          Get the faction of the segment inside the Eshelby
 *          particles
 *
 */
            factor[i] = 0.0;               
            intersection = SegPartListLookup(home, &node->myTag,
                                             &nbrNode->myTag);

            if (intersection != (SegPartIntersect_t *)NULL) {
               int ninc;
               int j, k, iCnt, numLimits, newRegion;
               real8 ratio[2];
 ...
 */
               ...
               for (iCnt = 0; iCnt < intersection->numIntersections; iCnt++) {
                  ...
                  int IsIntercept = IntersectionEllipseSegment(partPos, inclusion->radius,
 ...
 */
                  if (numLimits == 0) {
                     ...
                     if (newRegion == numLimits) {
                        ...
                        if (numLimits >= NUMLIMITS_MAX) {
                           Fatal("numLimits (%d) exceeds max of %d",
               ...
               for (j = 0; j < numLimits; j++) {
                  factor[i] += (1.0 - ratioList[0][j]) * (1.0 - ratioList[0][j]) -
                               (1.0 - ratioList[1][j]) * (1.0 - ratioList[1][j]);                  
               }

            } // end of the intersecting particles 
 ...
 */
                if (junctionExists[i]) {
                  ...
                  for (m = 0; m < 3; m++) {
                     fError[m] -= 0.5 * segTotLength[i] * fJunction[m] * (1-factor[i]);
                     
                     for (n = 0; n < 3; n++) {
                        dfErrordv[m][n] -= 0.5 * segTotLength[i] *
                                           dfJunctiondv[m][n] * (1-factor[i]);
                     }
                  }

                  // Inside particles
                  JunctionDrag(vTest, lineDir[i], dragLineE,
                               dragClimbE, fJunction, dfJunctiondv);
                  
                  for (m = 0; m < 3; m++) {
                     fError[m] -= 0.5 * segTotLength[i] * fJunction[m] * factor[i];

                     for (n = 0; n < 3; n++) {
                        dfErrordv[m][n] -= 0.5 * segTotLength[i] *
                                           dfJunctiondv[m][n] * factor[i];
                     }
                  }
#else
                    JunctionDrag(vTest, lineDir[i], dragLine,
 ...
 */
                    if (screwExists[i]) {
                       ...
                       for (m = 0; m < 3; m++) {
                            ...
                            fError[m] -= 0.5 * segScrewLength[i] * fScrew[m]* (1-factor[i]);

                            for (n = 0; n < 3; n++) {
                                dfErrordv[m][n] -= 0.5 * segScrewLength[i] *
                                                   dfScrewdv[m][n]* (1-factor[i]);
                            }
                        }
                       ...
                       ScrewDrag2(vTest, burg[i], glideDir[i],
                                 dragClimbE, dragLineE, dragScrewE,
                                 fScrew, dfScrewdv);

                       for (m = 0; m < 3; m++) {

                            fError[m] -= 0.5 * segScrewLength[i] * fScrew[m]* factor[i];

                            for (n = 0; n < 3; n++) {
                                dfErrordv[m][n] -= 0.5 * segScrewLength[i] *
                                                   dfScrewdv[m][n]* factor[i];
                            }
                        }
 ...
 */
                        ...
                        for (m = 0; m < 3; m++) {
                           ...
                           fError[m] -= 0.5 * segEdge1Length[i] * fEdge[m]* (1-factor[i]);

			    for (n = 0; n < 3; n++) {
			      dfErrordv[m][n] -= 0.5 *
				segEdge1Length[i] *
				dfEdgedv[m][n]* (1-factor[i]);
			    }
                        }
 ...
 */
                        ...
                        for (m = 0; m < 3; m++) {
                            ...
                            fError[m] -= 0.5 * segEdge2Length[i] * fEdge[m]* (1-factor[i]);
 ...
 */
                        EdgeDrag(vTest, burg[i], edgeDir1[i],
                                 dragClimbE, dragLineE, dragEdgeE,
                                 fEdge, dfEdgedv);

                        for (m = 0; m < 3; m++) {

                            fError[m] -= 0.5 * segEdge1Length[i] * fEdge[m]* factor[i];

			    for (n = 0; n < 3; n++) {
			      dfErrordv[m][n] -= 0.5 *
				segEdge1Length[i] *
				dfEdgedv[m][n]* factor[i];
			    }
                        }
 ...
 */
                        ...
                        for (m = 0; m < 3; m++) {
                            ...
                            fError[m] -= 0.5 * segEdge2Length[i] * fEdge[m]* factor[i];
 ...
 */
        ...
        if ((isnan(node->vX) || isinf(node->vX)) ||
            ...
            PrintNode(node);
            Fatal("Mobility_BCC_linear: (%d, %d) velocity error.",
