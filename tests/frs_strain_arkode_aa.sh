...
exe=../bin/paradis
dat=frs_strain.data
ctl=frs_strain_arkode_aa.ctrl
log=frs_strain_arkode_aa.log
dir=frs_strain_arkode_aa.rslts
...
maxstep=200             # maximum number of steps to run
...
if [ $sys == "osx" ] ; then
   mpirun -n $n $exe -maxstep $maxstep -d $dat $ctl | tee -a $log
fi
...
if [ $sys == "lc-linux" ] ; then
   salloc -n $n -c 1 -t $tmin -p pdebug srun -n $n $exe -maxstep $maxstep -d $dat $ctl | tee -a $log
fi

# parallel on lc sierra systems...

if [ $sys == "lc-sierra" ] ; then
   lalloc 1 -W $tmin -q pdebug jsrun -n$n -r$n -a1 -c1 $exe -maxstep $maxstep -d $dat $ctl | tee -a $log
fi
